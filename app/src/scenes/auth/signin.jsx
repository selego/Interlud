...
export default () => {
  ...
  const login = async () => {
    try {
      const { ok, code, user, token, userActionRights, collectivity, economicActor } = await api.post(`/user/signin`, values)
    ...
    <div className="relative overflow-hidden flex items-center justify-center py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-600">Accédez à votre espace InTerLUD+</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 space-y-6">
          ...
          >
            ...
            <div className="text-right mb-6">
              <Link to="/auth/forgot" className="text-sm text-primary-green hover:text-primary-green transition-colors">
                Mot de passe oublié ?
              </Link>
            </div>

            <button onClick={login} className="button-primary w-full">
              Se connecter
            </button>
          </form>
          ...
          <div className="text-center">
            <p className="text-sm text-gray-600">
              ...
              <Link className="text-primary-green hover:text-primary-green font-medium transition-colors" to="/auth/signup">
                Créer un compte
              </Link>
            </p>
          </div>
