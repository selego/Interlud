          ...
          <div className="space-y-2">
            {files && files.map((file) => (
              ...
              >
                ...
                <button
                  onClick={() => loadCells(file.id, file.name)}
                ...
                {cells.values.map((row, rowIndex) => (
                  <tr key={rowIndex}>
                    {row.map((cell, cellIndex) => (
                      <td 
                        key={cellIndex} 
                        className="border border-gray-300 px-4 py-2 bg-white text-center"
                      >
                        {cell}
                      </td>
                    ))}
                ...
                {cells.formulas.map((row, rowIndex) => (
                  <tr key={rowIndex}>
                    {row.map((cell, cellIndex) => (
                      <td 
                        key={cellIndex} 
                        className="border border-gray-300 px-4 py-2 bg-gray-50 text-center font-mono text-sm"
                      >
                        {cell || '-'}
                      </td>
                    ))}
