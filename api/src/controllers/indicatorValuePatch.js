...
const IndicatorValuePatch = require("../models/indicator_value_patch");
...
router.post("/search",passport.authenticate(["admin"], { session: false, failWithError: true }), async (req, res) => {
    try {
      ...
      const total = await IndicatorValuePatch.countDocuments(query);

      const data = await IndicatorValuePatch.find(query).sort({ date: -1 }).skip(skip).limit(limit).lean();
